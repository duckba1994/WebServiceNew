...
├── components/
│   ├── items/            # ItemTable, StatCard, RequestGrid (shared outgoing/incoming table), RequestDetailModal (per-module step tabs — `STEP_TABS_BY_MODULE`: IT / PL / CR; every workflow step's screen lives here as a panel), RequestActionDialog (confirm + dynamic form built from an action's requiredFields, then POST)
...
│   ├── useDeptMasterData.ts # GET /MasterData/{ga|im|af|sv|sqa|ps} — one contract for all five (sections/types/requestTypes/requestSubTypes/units); still pending backend (MdApi/API_SPEC_DEPT_MASTER.md)
...
│   ├── crRequestForm.ts  # PURE form→payload mapping for the CR request (requestDate = "วันที่ต้องการ"; requestBy/departid/jobDate(=DocDate, วันที่สร้างเอกสาร) left to the backend)
...
- **Dropdown options come from master data, never from a list typed into the code**: each department's option sets live behind one endpoint (`/MasterData/it` · `/MasterData/pl` · `/MasterData/cr`) with a hook per endpoint. GA / IM / AF / SV / SQA / PS instead share **one contract** (`DeptMasterDataApi`: `sections` / `types` / `requestTypes` / `requestSubTypes` / `units` / `estimates`) behind `/MasterData/{dept}`, so they share **one hook** (`useDeptMasterData`) and the generic `master` keys `deptSections` / `deptTypes` / `deptRequestTypes` / `deptRequestSubTypes`; a row's `section` / `typeId` are optional and the hook filters by them only when the API actually sends them (a department with no ส่วนงาน is unaffected). A new department of this shape needs only a `DEPT_FORMS` entry plus its short name in `DEPT_MASTER_DEPTS` (`CreateItem.tsx`) — no new hook. Rules: **the value stored in the form and sent to the API is the option's `name`, not its `id`** (the request APIs store names, and `resolution`/list rows come back as plain text — an id would render as a number); the one exception is CR's ส่วนงาน, which stores the section `code` because `requestTypes`/`requestSubTypes` join on it. **Never ship a hardcoded fallback list** — if the fetch fails, show the API's error plus a "ลองใหม่" button under the field (a list typed by hand drifts from what the backend actually stores, and a mismatched name is written straight into the DB). A saved value that is no longer in the master list must still render as a selected option, otherwise the select silently goes blank and the next save wipes it. Chained lists (IT สาเหตุหลัก→สาเหตุรอง, CR ส่วนงาน→ประเภท→รายละเอียด) clear their children when the parent changes; CR's ids repeat across sections (HV id 1 ≠ FL id 1) so every lookup must filter by section as well. In the create form the chain is declared as data on the field (`master` / `dependsOn` / `resets` in `FieldDef`), not as per-department code in the page.

- **A field can be filled by another field's selection**: an option may carry a whole record (`FieldOption.data`) and the picking field lists the keys it writes (`FieldDef.fills`) — PS's เลขที่ใบประเมินราคา fills the 9 machine fields below it, which are `kind: 'filled'` (read-only; the source document is the only place to change them). Picking another document overwrites the whole set and clearing the pick clears the whole set, so two documents' data can never end up side by side.
...
- **Each workflow step is a tab, not a page**: there is no route per step. `RequestDetailModal` renders a stepper whose tabs come from `STEP_TABS_BY_MODULE` (IT 6 / PL 5 / CR 5, each counting General) and each tab owns one panel — that is the "หน้ารับเรื่อง / หน้าดำเนินการ / หน้ารับงาน / หน้าปิดงาน" of that department. **The approve step deliberately has no tab of its own in any module** (per user decision, 1 Sep 2026 for CR): approving happens at the bottom of the General tab, next to what is being approved. A tab's `actionCodes` decides which of `availableActions` may surface there (`panelCodes` = claimed by the tab but rendered by its panel, because the button carries form values); anything a module's tabs don't claim falls through to the General tab so a button is never lost — which is exactly why `approve` must stay unclaimed, and why `claimedCodesOf(module)` is computed **per module** rather than as one global set. A tab's step number is resolved from the API's `workflow.steps` by `wfCodes` (WFStatus), and the number written in the code is only the fallback while the detail request is in flight — so the backend can renumber or insert steps without a frontend deploy.

- **CR workflow (5 steps)** — contract: `CR-workflow-frontend-guide.md` (backend repo), local notes in `MdApi/API_SPEC_CR_FLOW.md`. อนุมัติ `approve` (Mgr ต้นสังกัด) → รับเรื่อง `receive` + **required `requestService`** (CR) → ดำเนินการ `saveService` (saves as often as you like, **stamps no name/date and never moves the step**) / `service` + required `serviceDetail` (CR) → **รับงาน `acceptWork` (หน่วยงานผู้แจ้ง — requester side; steps 1 and 4 are stored with the requester's departId, so they stall silently unless "รอเราลงมือ" in the outbox catches them)** → ปิดงานที่แจ้งเรื่อง `close` + optional `actionDetail` (CR). Traps that will bite anyone editing this: the field names are **deliberately asymmetric** — you send `serviceDetail` but read it back at `resolution.resolutionDetail`; CR reuses `type` for ส่วนงาน (HV/FL) and `requestType` for "ประเภท / จัดทำ" (no separate sub-type, no `planDate`); a closed CR ticket comes back as `wfStep` 6 of 5 **on purpose** (so no department holds it), which is why `stepText()` shows the phase label instead once `wfStep > wfStepTotal`; and the only English error message in the whole API is CR's 404, translated in `useRequestDetail.ts`. Still missing from the API (frontend already leaves room for them): ไม่อนุมัติ / ยกเลิก / ไฟล์แนบ. **Editing a CR ticket goes through its own pair of endpoints, not the central one**: `GET /CRRequest/{docNo}` (via `useCrRequest`) is the only source that returns ส่วนงาน / ประเภทที่แจ้ง / รายละเอียดที่แจ้ง as separate values — the central `/Requests/CR/{docNo}` fuses the last two into one `requestType` string (`"ประเภท / จัดทำ"`) that cannot drive a dropdown, so a failed load disables the save button rather than letting the form overwrite the ticket's type with blanks. Only three fields are editable — `requestSubType` / `requestDate` / `requestDetail` (plus `requestSubOther`, gated by `showWhen` on `EditFieldDef`); **`section` and `requestType` are deliberately read-only** even though the API accepts them, because the ticket's running number was issued from that pair and does not change when they do, so an edit would leave the ticket holding a number from a different series (per user decision, 1 Sep 2026 — the earlier version allowed all five with a warning). Both still travel in the PUT with their original values since the API requires them, and `requestSubType`'s options stay filtered by the ticket's own (section + requestType). Two rules the backend does **not** enforce yet, so the frontend's version is cosmetic until it does (`MdApi/API_SPEC_CR_FLOW.md` §4.1): editing is blocked once Mgr approves, and only the requester's department may edit — the API's `canEdit` is false only for closed/cancelled tickets.  Attachments: the รูปภาพ block and the upload slots render only when `attachmentApiOf(module).slots` is non-empty — CR has no attachment endpoint, so the whole block is hidden rather than reading "ไม่มีรูปแนบ" on every ticket forever.
...
## Related Docs
`MdApi/API_SPEC_DEPT_MASTER.md` is the contract handed to backend for the GA/IM/AF/SV/SQA option lists — it also lists what is still missing for those departments (the create-request endpoints, and SV's attachment endpoint), which is why their forms end at the summary screen without POSTing.
