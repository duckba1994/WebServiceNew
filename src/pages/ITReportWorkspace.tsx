...
import {
  IconAlertTriangle, IconArrowLeft, IconChartBar, IconClipboardList,
  IconLoader2, IconPrinter, IconSearch,
    ...
    <Layout title={report.title} subtitle={`${report.department} Report`}>
      <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-gray-200 px-5 py-3 dark:border-slate-700">
          ...
          <button type="button" disabled={!current.data || current.loading} onClick={() => window.print()} className="ml-auto inline-flex items-center gap-2 rounded-lg border border-gray-200 px-3.5 py-2 text-[12.5px] font-semibold text-slate-600 transition hover:border-accent hover:text-accent disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:text-slate-300"><IconPrinter size={16} />พิมพ์รายงาน</button>
        </div>
        ...
        <div className="relative min-h-0 flex-1">
          {current.loading && !current.data ? <ReportState icon={<IconLoader2 size={28} className="animate-spin" />} text="กำลังโหลดรายงาน…" />
            ...
            : isSurvey ? <SurveyPrint data={survey.data!} preview /> : <ServicePrint data={service.data!} preview />}
        </div>
      </section>
